...
The Radiological Search Model (RSM) is detailed in my [FROC paradigm book](https://dpc10ster.github.io/RJafrocFrocBook/). A brief summary follows: the RSM has three non-threshold related parameters: $\mu$, $\lambda$ and $\nu$ and as before $\zeta$ is a sliding threshold that determines the operating point. One also needs to specify the lesion distribution vector $\overrightarrow{f_L})$, basically the fraction of diseased cases with one lesion, the fraction with two lesions, etc, and let $R_{FROC}$ denote the number of bins in the FROC rating study.


Early efforts at estimating parameters of the RSM revealed that an FROC curve based estimation method patterned after [@edwards2002maximum] worked only for designer level CAD data and not for human observer data. Subsequent effort focused on ROC curve-based fitting, and this proved successful at fitting radiologist datasets where detailed definition of the ROC curve is not available. A preliminary account of this work is in  [@chakraborty2011estimating]. 
...
The total number of parameters to be estimated, including thresholds, is $3+R_{FROC}$. Maximizing the likelihood function yields estimates of the RSM parameters. 
...
The lesion distribution vector is `r lesDistr`. This means that fraction `r lesDistr[1]` of diseased cases contain one lesion, fraction `r lesDistr[2]` contain two lesions and fraction `r lesDistr[3]` contain three lesions. The lesion distribution vector is needed because the fitted curve depends on the lesion distribution. For example, the ROC curve will be lower if each diseased case contains one lesion than if each diseased case contains three lesions.   
...
The fitting program decreased the negative log-likelihood (NLL) from `r round(ret$NLLIni,2)` to `r round(ret$NLLFin,2)`. A decrease in negative log-likelihood is expected since one is maximizing the log-likelihood.  
