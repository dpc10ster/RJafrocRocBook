...
\footnote{In the univariate case, \(p = 1\), \(\Sigma\) is replaced by \(\sigma^2\), and the above expression reduces to the pdf of the normal distribution \(N(\mu, \sigma^2)\).}

The \texttt{R} function to evaluate Eqn. \eqref{eq:bivariate-binormal-model-density-function-multivariate} is \texttt{dmvnorm()}, for ``density of multivariate normal distribution'', available via \texttt{R} package \texttt{mvtnorm}. Its usage is illustrated next for the special case of a bivariate distribution and the following parameter values:
...
\right) \\
\end{aligned}\right\}
\label{eq:bivariate-binormal-model-mu-parameters}
\end{equation}

The indices are \(it\), i.e., \emph{modality followed by the truth index}. The equations state that the two non-diseased distributions, one per modality, are both centered on zero, while the diseased distribution for modality 1 is centered at \(\mu_1\), shorthand for \(\mu_{12}\), while that for modality 2 is centered at \(\mu_2\), shorthand for \(\mu_{22}\).

The covariance matrices \(\Sigma_1\) and \(\Sigma_2\) in Eqn. \eqref{eq:bivariate-binormal-model-sampling} are defined by:
...
\right)
\end{aligned}\right\}
\label{eq:bivariate-binormal-model-covariance-matrices}
\end{equation}
