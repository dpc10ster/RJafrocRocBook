...
\hypertarget{bivariate-models}{%
\chapter{Bivariate binormal model}\label{bivariate-models}}

\hypertarget{bivariate-models-how-much-finished}{%
\section{How much finished 30\%}\label{bivariate-models-how-much-finished}}

\hypertarget{bivariate-models-introduction}{%
\section{Introduction}\label{bivariate-models-introduction}}
...
In \citep{chakraborty2017observer} Chapter 21 was devoted to the CORROC2 algorithm and usage of the software was described in detail. At that time \citep{zhai2017bivariate} was in press - this paper describes an alternate algorithm -- termed the correlated contaminated binormal model or CORCBM -- that has several advantages over CORROC2. Therefore, I have reconsidered the focus of this chapter: it now outlines the correlated binormal model fitting procedure but does not detail how to use it. Instead I now include a detailed explanation of the work presented in \citep{zhai2017bivariate} and how to use the CORCBM software implementing it. The included description of the CORROC2 model - basically how to extend the binormal model to a correlated binormal model - will help the reader better understand how to extend the contaminated binormal model to the correlated contaminated binormal model.
...
\hypertarget{bivariate-models-binormal}{%
\section{The bivariate binormal model}\label{bivariate-models-binormal}}

\hypertarget{the-bivariate-normal-distribution}{%
\subsection{The bivariate normal distribution}\label{the-bivariate-normal-distribution}}
...
\hypertarget{bivariate-models-binormal-density}{%
\subsection{The bivariate normal density function}\label{bivariate-models-binormal-density}}
...
\label{eq:bivariate-models-binormal-density-function}
\end{equation}
...
\label{eq:bivariate-models-binormal-density-function2}
\end{equation}
...
\sigma_2^2
\end{matrix}
 \right)\label{eq:bivariate-models-binormal-covariance-matrix}
\end{equation}
...
The \texttt{R} function to evaluate Eqn. \eqref{eq:bivariate-models-binormal-density-function} is \texttt{dmvnorm()}, for ``density of multivariate normal distribution'' available via \texttt{R} package \texttt{mvtnorm} \citep{R-mvtnorm}. Its usage is illustrated next for the following parameter values:
...
\label{eq:bivariate-models-binormal-parameters}
\end{equation}
...
\hypertarget{bivariate-models-binormal-formulae}{%
\subsection{The bivariate binormal model}\label{bivariate-models-binormal-formulae}}
...
\right)
\end{aligned}\right\}
\label{eq:bivariate-models-binormal-all-parameters}
\end{equation}
...
z_{2k_tt}
\end{matrix} \right)
\label{eq:bivariate-models-binormal-notation}
\end{equation}
...
The sampling model Eqn. \eqref{eq:bivariate-models-binormal-sampling} is a natural extension to two modalities of the single modality univariate binormal model described in Chapter \ref{binormal-model}:
...
\hypertarget{bivariate-models-binormal-multivariate-density-visualization}{%
\subsection{Visualizing the bivariate binormal density functions}\label{bivariate-models-binormal-multivariate-density-visualization}}

It is helpful to visualize the pdfs. Since one is dealing with a vector \(\overrightarrow{x}\) of length 2, one needs two axes to depict \(x_1\) and \(x_2\) and a third axis to depict the pdf. The R-package \texttt{plotly} \citep{R-plotly} provides a visualization technique. It is interactive: by dragging the cursor over the plot one can visualize it from different angles. Shown in Fig. \ref{fig:bivariate-models-binormal-pdf-plots} are four views of the bivariate distribution density function for \(\mu_1 = 1.5\), \(\mu_2 = 2.0\), \(\sigma_1 = 1.0\), \(\sigma_2 = 1.5\), \(\rho_1 = 0.3\) and \(\rho_2 = 0.6\).

\begin{figure}
\centering
\includegraphics{10-bivariate-binormal-model_files/figure-latex/bivariate-models-binormal-pdf-plots-1.pdf}
\caption{\label{fig:bivariate-models-binormal-pdf-plots}Bivariate pdf plots for the bivariate distribution density function for \(\mu_1 = 1.5\), \(\mu_2 = 2.0\), \(\sigma_1 = 1.0\), \(\sigma_2 = 1.5\), \(\rho_1 = 0.3\) and \(\rho_2 = 0.6\).. Top-left is the default view. Top-right is the `birds eye' view: the lower-left blob is the diseased distribution. Bottom-left is the view looking along the y-axis: the left distribution is non-diseased. Bottom-right is the view looking along the x-axis: the higher peak distribution is non-diseased.}
\end{figure}

\hypertarget{bivariate-models-binormal-multivariate-density-estimation}{%
\subsection{Estimating bivariate binormal model parameters}\label{bivariate-models-binormal-multivariate-density-estimation}}
...
\hypertarget{bivariate-models-contaminated-binormal-density}{%
\section{The bivariate contaminated binormal model}\label{bivariate-models-contaminated-binormal-density}}
...
\hypertarget{bivariate-models-contaminated-binormal}{%
\subsection{The bivariate contaminated binormal model}\label{bivariate-models-contaminated-binormal}}

The bivariate contaminated binormal model (BCBM) is the bivariate extension of the univariate contaminated binormal model described in Chapter \ref{proper-roc-models}. Corresponding to the two truth states the BCBM is defined by two bivariate distributions. Following the notation in Eqn. \eqref{eq:bivariate-models-binormal-notation}, for case \(k_tt\) the bivariate distribution yields two samples \(z_{1k_tt}\) and \(z_{2k_tt}\).

\hypertarget{non-diseased-cases}{%
\subsubsection{Non-diseased cases}\label{non-diseased-cases}}

For non-diseased cases the sampling is as follows:

\begin{equation}
\overrightarrow{z_{k_11}} \sim  N_2\left( \overrightarrow{\mu_1}, \Sigma_1 \right)
\label{eq:bivariate-models-binormal-sampling-teq1}
\end{equation}
...
\def\labelenumi{\arabic{enumi}.}
\item
  The disease is not visible in either modality. The appropriate distribution, denoted \(N_{00}\), is a bivariate \(N_2\) distribution centered at \((0,0)\) and has correlation \(\rho_{00}\) and mixing fraction \((1-\alpha_1)(1-\alpha_2)\).
\item
  The disease is visible in modality 1 but not visible in modality 2. The appropriate distribution, denoted \(N_{10}\), is a bivariate \(N_2\) distribution centered at \((\mu_1,0)\) and has correlation \(\rho_{10}\) and mixing fraction \(\alpha_1(1-\alpha_2)\).
\item
  The disease is not visible in modality 1 but is visible in modality 2. The appropriate distribution, denoted \(N_{01}\), is a bivariate \(N_2\) distribution centered at \((0,\mu_2)\) and has correlation \(\rho_{01}\) and mixing fraction \((1-\alpha_1)\alpha_2\).
\item
  The disease is visible in both modalities 1 and 2. The appropriate distribution, denoted \(N_{11}\), is a bivariate \(N_2\) distribution centered at \((\mu_1,\mu_2)\) and has correlation \(\rho_{11}\) and mixing fraction \(\alpha_1\alpha_2\).
\end{enumerate}
...
\end{matrix}
\end{aligned}\right\}
\label{eq:bivariate-models-cbm-diseased}
\end{equation}
...
\end{aligned}\right\}
\label{eq:bivariate-models-cbm-pdf-diseased}
\end{equation}
...
\hypertarget{bivariate-models-binormal-corroc2-discussion}{%
\section{Discussion / Summary}\label{bivariate-models-binormal-corroc2-discussion}}
