...
## The bivariate binormal model {#bivariate-models-contaminated-binormal-density}

The bivariate binormal model is the bivariate extension of the univariate contaminated binormal model described in Chapter \@ref(proper-roc-models). For non-diseased cases the pdf is idential to that of the correlated binormal model (i.e., that used in CORROC2). 
...
The `R` function to evaluate Eqn. \@ref(eq:bivariate-binormal-model-density-function) is `dmvnorm()`, for “density of multivariate normal distribution” available via `R` package `mvtnorm` [@R-mvtnorm]. Its usage is illustrated next for the following parameter values:
...
<!-- In Eqn. \@ref(eq:bivariate-binormal-model-sampling), the symbol $\sim  N_2\left( \overrightarrow{\mu_t}, \Sigma_t \right)$ denotes a vector sample of length two from the bivariate normal distribution with mean $\overrightarrow{\mu_t}$ and covariance matrix $\Sigma_t$.  -->
...
## The bivariate contaminated binormal model {#bivariate-contaminated-binormal-model-density}
