...
## How much finished 80% {#ratings-paradigm-how-much-finished}
...
```{r ratings-paradigm-example-table, echo=FALSE}
Ktr = array(dim = c(2,5))
...
```{r ratings-paradigm-table-2, echo=FALSE}
options(digits=4)
# R = number of bins; 5
R <- length(Ktr[1,]) 
...
```{r ratings-paradigm-labeling-points-code, echo=FALSE}
K1 <- 60;K2 <- 50
...
```{r ratings-paradigm-labeling-points-figure, fig.cap="Operating point labeling convention", fig.show='hold'}
grid::grid.draw(p)
```
...
>To summarize, the operating points are labeled starting with the upper right corner, labeled $O_2$, and working down the curve each time increasing the number by one. The total number of non-trivial points is $R-1=4$. 
...
Notice that the values of the arrays `FPF` and `TPF` are identical to those listed in Table \@ref(tab:ratings-paradigm-table-2). Regarding lines 10 and 11 of code it was shown in Chapter \@ref(binary-task) that in the equal variance binormal model the operating point determines the parameters $\mu_{ev}$ = `r mu_ev`, Eqn. \@ref(eq:binary-task-model-solve-mu), or equivalently $A_{z;\sigma = 1}$ = `r Az_ev`, Eqn. \@ref(eq:binary-task-model-az-var). These lines illustrate the application of these formulae using the coordinates (0.5, 0.9) of the uppermost non-trivial operating point, i.e., one is fitting the equal variance model to the uppermost operating point.

Shown next is the equal-variance model fit to the uppermost non-trivial operating point, left plot, and for comparison, the right plot is the unequal variance model fit to all operating points (the unequal variance model is described in the next chapter).
...
```{r ratings-paradigm-eq-var-fit-a, fig.cap="(A): The left figure is the predicted ROC curve for $\\mu=1.28$ superposed on the operating points. (B): The right figure is the same data fitted with a unequal variance model described in the next chapter.", fig.show='hold'}
grid.arrange(p1,p2,ncol=2)
```


It should come as no surprise that the uppermost operating point in plot A is *exactly* on the predicted curve: after all, this point was used to calculate $\mu_{ev}$ = `r mu_ev`. The corresponding value of $\zeta$ can be calculated from Eqn. \@ref(eq:binary-task-model-se-sp), namely:
...
* Fig. \@ref(fig:ratings-paradigm-eq-var-fit-a) (B) shows the predicted ROC curve by the unequal variance binormal model, to be introduced in the next chapter. The corresponding parameter values are $\mu$ = `r mu` and $\sigma$ = `r sigma`. 
...
```{r SixPointScale, echo=FALSE, fig.cap="A method for acquiring ROC data on an effectively 6-point scale that also yields an unambiguous single operating point for declaring patients diseased. Note the reversal of the ordering of the final ratings in the last \"column\" in the lower half of the figure.", fig.show='hold', fig.align='center'}
...
```{r 2AFC, echo=FALSE, fig.cap="Example of image presentation in a 2AFC study.", fig.show='hold', fig.align='center'}
