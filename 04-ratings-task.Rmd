...
```{r ratings-paradigm-labeling-points-figure, fig.cap="Operating point labeling convention and the empirical ROC plot.", fig.show='hold', echo=FALSE}
grid::grid.draw(p)
```
...
```{r ratings-paradigm-eq-var-fit, fig.cap="A: This panel is the predicted ROC curve for $\\mu=1.28$ superposed on the operating points. B: This panel is the same data fitted with a unequal variance model described in a following chapter.", fig.show='hold'}
grid.arrange(p1,p2,ncol=2)
```


It should come as no surprise that the uppermost operating point in panel A is *exactly* on the predicted curve: after all, this point was used to calculate $\mu_{ev}$ = `r mu_ev`. The corresponding value of $\zeta$ can be calculated from Eqn. \@ref(eq:binary-task-model-se-sp), namely:
...
* Fig. \@ref(fig:ratings-paradigm-eq-var-fit) panel B shows the predicted ROC curve by the unequal variance binormal model, to be introduced in a following chapter. The corresponding parameter values are $\mu$ = `r mu` and $\sigma$ = `r sigma`. 
