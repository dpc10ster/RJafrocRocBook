...
A related issue is data degeneracy that occurs when a reader fails to provide an interior ROC point. The binormal model does not yield reasonable fits to such data and the researcher has no option but to discard this data or use the empirical AUC. 

> Needed first is an understanding of a quantity called the likelihood ratio and demonstration that the slope of the true (???) ROC curve always equals the likelihood ratio, independent of parametric assumptions.
...
An observer who maximizes TPF for a given FPF for all values $0 \le \text{FPF} \le 1$, is called a Neyman-Pearson observer [@neyman1933ix]. It can be shown that the Neyman-Pearson observer uses the likelihood ratio as the decision variable [@green1966signal]. 

> A consequence of the theorem is that AUC for a likelihood ratio observer represents an upper limit on performance, and such an observer is called an "ideal observer" use of the likelihood ratio as the decision variable always yields proper ROC curves.
...
So far the decision rule has been that if the z-sample for a case is equal to or greater than threshold $\zeta$ the case is diagnosed as diseased, and otherwise it is diagnosed as non-diseased. 

The likelihood ratio involves a deeper concept. It is assumed that the observer knows the pdfs of non-diseased and diseased cases as functions of z. Assuming equal numbers of diseased and non-diseased cased the observer uses the observed z-sample and knowledge of the value of the two pdfs at the observed value of z to compute the ratio $l(z)$. If the ratio is equal or greater than some preset value $\lambda$, the case is diagnosed as diseased and otherwise the case is diagnosed as non-diseased.


A word on notation: when the decision variable was $z$, I used the corresponding Greek character $\zeta$ to denote a threshold on that axis. Since likelihood ratio is denoted $l$, the corresponding Greek character is $\lambda$.


So in effect, the observer has transformed the decision variable $z$ to a new variable $l$ defined by $l \equiv \frac{\text{pdf}_D\left( z \right)}{\text{pdf}_N\left( z \right)}$ and the new variable $l$ is used for making decisions. Varying $\lambda$ allows the observer to move along the resulting *likelihood ratio ROC curve*, which has the "proper" property. 


If the decision variable z results in improper ROC curves, as does the z-sample in the binormal model, the transformation will not be monotonic ^[For a positive-directed decision variable, as in this book, the term "monotonic" is always meant to be "monotonic increasing". It is possible for a transformation to be monotonic decreasing, as in $l = 1/z$.]. If the transformation were monotonic, that would imply that z was already equivalent to the likelihood ratio since a monotonic transformation exists that relates the two. The ROC curve is invariant to monotonic transformations of the decision variable. 
...
This is worthy of some illustrations: Fig. \@ref(fig:proper-roc-models-plots-2) plots A-F, all of which apply to $a = 0.7$ and $b = 0.5$. The ordinate is labeled $\text{lr}$ for likelihood ratio. 
...
For now focus on Plot A in Fig. \@ref(fig:proper-roc-models-plots-2) in which range of z is: -5 to 8.  Imagine a sliding threshold $\lambda$ moving down along the ordinate, starting from very high values, $\approx 1.5 \times 10^{11}$. The decision rule is to diagnose the case as "diseased" if $\text{lr} > \lambda$, and otherwise the case is diagnosed as "non-diseased". In Plot A the likelihood ratio $\text{lr}$ seems, for all practical purposes, to be monotonically related to z, so on the surface is makes no difference which decision rule is used, that based on $\text{lr} \ge \lambda$ or the one based on $z \ge \zeta$. The same conclusion is reached for Plot B, which focuses on the range (-3,5). Plot C reveals the first hint of something different going on [it shows the range (-3,3)], for now the ordinate $l$ is not monotonically related to $z$, rather the function $l(z|a,b)$  has a minimum. For values of $\text{lr}$ below approximately 1 there are two values of $\zeta$ corresponding to a single value of $\lambda$. This behavior is amplified in the remaining plots, as one "homes" in on the minimum. Taking the clearest example, Plot F, for $\lambda = 0.37$ the two values are (-0.728, 0.206). The two values are denoted by $\zeta_L=-0.728$ and $\zeta_U=0.206$. The likelihood ratio observer, using threshold $\lambda=0.37$ declares all cases with $\text{lr} \ge 0.37$ as "diseased" and "non-diseased" otherwise. The corresponding z-sample binormal model observer must declare cases with $z \ge \zeta_U$ and cases with $z < \zeta_L$ as diseased and "non-diseased" otherwise. The z-sample rule is non-intuitive: why should cases with very low z-samples be declared diseased? The reason is that the z-sample based model assumed by the observer (as noted earlier, the analysis assumes that the observer "knows" or "learns" the two distributions and makes rational decisions based on this knowledge) is itself unphysical. According to the binormal model both very high and very low values of z are consistent with the case being diseased. If the observer is rational, then he will abandon making decisions based on $z$ and instead use the transformation $\text{lr}=\text{lr}(z|a,b)$ to make decisions based on $\text{lr}$. The $\text{lr}$-based decision rule is intuitive: if $\text{lr} > \lambda$ one diagnoses the case as "diseased", otherwise one diagnoses it as "non-diseased".


> Since likelihood ratio decreases monotonically as $\lambda$ is reduced (i.e., one is moving down the y-axis in Fig. \@ref(fig:proper-roc-models-plots-2) the slope ROC curve decreases monotonically as one moves up the curve. One needs to think carefully about this sentence to eventually "get it". In other words, the likelihood ratio observer generates a proper ROC curve. 


Another theorem14,16,18, is that the likelihood ratio observer achieves highest AUC when compared to other observers using other decision rules. Specifically, the likelihood ration observer achieves higher AUC than one using the binormal model z-sample exceeding a threshold. For the binormal model based likelihood ratio observer, the slope will not decrease to zero, rather it will decrease to the limiting value, 0.361, suggested by \@ref(fig:proper-roc-models-plots-2) Plot F (for values of $\lambda$ below the minimum, there are no solutions for z). 


Since likelihood ratio is the ratio of two probabilities, it - and consequently the slope of the ROC -  is always non-negative. This rules out chance line crossings and "hooks".
...
A final theorem: since AUC of the likelihood ratio observer is optimal with respect to other observers using different decision rules or different underlying models of the decision variable, *the AUC achieved by the likelihood ratio observer is unique, regardless of the model used to fit the proper ROC curve.* In other words, the shapes of the predicted proper ROC curves could be model dependent and differ from each other but the AUCs will be the same. This is demonstrated in book Chapter TBA 18 for RSM, PROPROC and CBM fitted AUCs.
