...
The bivariate binormal model is a straightforward extension of the bivariate model just described. The extension is to *two truth-states*, non-diseased and diseased. This could result in a doubling of the number of parameters from 6 to 12, but by appropriate shift and scaling transformations one can ensure that the two means of the bivariate distribution for non-diseased cases are zeroes and the two variances are unity. This leaves the following parameters for the non-diseased cases:    
...
The sampling for each modality, considered by itself, is as follows (the parameters $\mu$ and $\sigma$ are of course modality dependent):
