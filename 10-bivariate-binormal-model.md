...
## TBA How much finished 20% {#bivariate-binormal-model-how-much-finished}
...
One might wonder why even bother describing the older approach ^[The significance testing methods assume a figure of merit can be calculated and address how to perform significance testing using that figure of merit.]. Before bringing in the RSM, addressed in TBA book Chapter 18 it is helpful to understand the paired binormal model applicable to paired ROC data. This handles two ratings per case, one per reader, and the analogous situation when one has a single reader interpreting cases in two modalities. 
...
For the special case of a bivariate normal distribution $p = 2$ this is equivalent to (the subscript 2 is suppressed) ^[In the univariate case, $p = 1$, $\Sigma$ is replaced by $\sigma^2$, and the above expression reduces to the pdf of the normal distribution $N(\mu, \sigma^2)$.]:
...
In the following code the call to get the pdf occurs at line: `pdf <- dmvnorm(x, mean = c(mu1,mu2), sigma = Sigma)`.
...
The parameters are truth-state dependent, Eqn. \@ref(eq:bivariate-binormal-model-all-parameters). In addition one needs to include threshold parameters in the modeling. 
