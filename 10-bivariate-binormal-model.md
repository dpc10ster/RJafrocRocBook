...
Recall, Eqn. \@ref(eq:proper-roc-models-cbm-pdfd), that for the univariate CBM for diseased cases the pdf is a mixture of two unit variance distributions, one centered at zero and the other at $\mu$ with mixing fractions $(1-\alpha)$ and $\alpha$, respectively. For diseased cases the BCBM needs to account for the possibility that the mixing fractions can be different in the two modalities: these are modeled by $\alpha_1$ and $\alpha_2$. Likewise, the z-sample means for cases where the disease is visible can be different in the two modalities: these are modeled by $\mu_1$ and $\mu_2$. All distributions are unit variance but can have different correlations as described next. 

Four types of mixings and the corresponding bivariate distributions need to be accounted for:
...
The means vectors and covariance matrices for the four types of mixings are defined by:
...
In this equation $p$ denotes the bivariate normal pdf function with the specified mean-vectors and covariance matrices.  
