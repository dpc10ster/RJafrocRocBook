...
```{r binormal-model-fit, fig.cap= "Operating points and fitted binormal ROC curve. The fitting values are $a = 1.3205$ and $b = 0.6075$. Confidence intervals are shown for the lowest and uppermost non-trivial points.", fig.show='hold', echo=TRUE}
...
```{r improper-roc, fig.cap= "ROC curves for different parameters. The curve 1 corresponds to $a = 0.7, b = 0.5$ and shows the clearest example of chance line crossing. Curve 2 corresponds to $a = 0.7, b = 1.5$ and it shows chance line crossing near the origin. Curve 3 line corresponds to $a = 1.5, b = 0.5$. Curve 4 corresponds to $a = 2, b = 0.5$. All curves exhibit chance line crossings even though they may not be clearly evident.", fig.show='hold', echo = FALSE}
print(p)
```
...
To understand this, consider that the non-diseased distribution pdf is a Dirac delta function centered at zero (this function integrates to unity) and the unit variance diseased distribution is centered at 0.6744898.  In other words this model is characterized by $a = 0.6744898$ and $b = 0$.  As the threshold $\zeta$ is moved from the far right to the left, TPF will increase but FPF is stuck at zero until $\zeta$ reaches zero. Just before reaching this point, the coordinates of the ROC operating point are (0, 0.75). The 0.75 is due to the fact that $z = 0$ is -0.6744898 units relative to the center of the diseased distribution, so the area under the diseased distribution to the left of $z = 0$ is `r simplePrint(pnorm(-0.6744898))`. Since `pnorm` is the probability below the threshold, TPF must be its complement, namely `r 1-pnorm(-0.6744898)`. This explains the operating point (0,0.75) which lies on the y-axis. As the threshold crosses the delta function FPF shoots up from 0 to 1 but TPF stays constant. Therefore, the operating point has jumped from (0, 0.75) to (1, 0.75). When the threshold is reduced further, the operating point moves up vertically, along the right side of the ROC plot, until the threshold is so small that virtually all of diseased distribution exceeds it and the operating point reaches (1, 1). The ROC curve is illustrated in plot A. 
  
```{r, binormal-model-degenerate-datasets-example1, fig.align = "center", fig.cap="An extreme example of a binormal fit with a hook.", fig.show='hold', echo = FALSE}
...
Actually, given one operating point (0, 0.75) the preceding fit is not even unique. If the diseased distribution is shifted appropriately to the right of its previous position, and one can determine the necessary value of $a$, then the ROC curve will shoot upwards through the operating point (0, 0.75) to (0, 0.9), as in plot B, before proceeding horizontally to (1, 0.9) and then completing the curve vertically to (1, 1).  If the diseased distribution is shifted well to the right, i.e., $a$ is very large, then the ROC curve will shoot upwards past the operating point, as in plot C, all the way to (0,1) before proceeding horizontally to (1, 1).

```{r binormal-model-degenerate-datasets-example2, fig.align = "center", fig.cap="The exact fit is not unique.", fig.show='hold', echo = FALSE}
a <- 1.281552; b <- 0
plotCurve <- BMPoints(a, b)
...
```{r binormal-model-degenerate-datasets-example3, fig.align = "center", fig.cap="The exact fit is not unique.", fig.show='hold', echo = FALSE}
a <- Inf; b <- 0
plotCurve <- BMPoints(a, b)
...
```{r binormal-model-degenerate-datasets-example4, fig.align = "center", fig.cap="A reasonable fit to a degenerate dataset.", fig.show='hold', echo = FALSE}
mu <- Inf; alpha <- 0.75
...
```{r binormal-model-degeneracy, fig.cap="Panels A, B and C illustrate that the exact fit is not unique. Panel D illustrates a reasonable unique fit to the data.", fig.show='hold', echo=FALSE}
grid.arrange(grobs = list(figA, figB, figC, figD),
             nrow =2,ncol=2)
```


Panels A, B and C in Fig. \@ref(fig:binormal-model-degeneracy) represent exact fits to the observed operating point with $b = 0$ and different values of $a$. None of them is reasonable. Panel D is discussed below.


### Comments on degeneracy  {#binormal-model-comments}

Degeneracy occurs if the observer does not provide any interior operating points. So why worry about it? Perhaps one has a non-cooperating observer, who is not heeding the instructions to *spread the ratings, use all the bins*. The following example shows that the observer could in fact be cooperating fully and still be unable to provide any interior data points. Consider 100 diseased cases consisting of 75 easy cases and 25 difficult cases and 100 easy non-diseased cases. The observer is expected to rate the 75 easy diseased cases as *fives*, the difficult diseased cases as *ones* and the 100 easy non-diseased cases as *ones*. No amount of coaxing to spread their ratings is going to convince this observer to rate with twos, threes and fours any of the 75 easy diseased cases. If the cases are obviously diseased, and that is what is meant by *easy cases* they are supposed to be rated fives, i.e., definitely diseased. Forcing them to rate some of them as *probably diseased* or *possibly diseased* would be irrational and guilty of bending the reading paradigm to fit the convenience of the researcher, never a good idea.


### A reasonable fit to the degenerate dataset {#binormal-model-reasonable-fit}

If the dataset yields a single operating point (0, 0.75), what is a reasonable ROC plot? A theorem states that given an observed operating point, the line connecting that point to (1, 1) represents a lower bound on achievable performance by the observer. The observer achieves this lower bound by guessing to classify the remaining cases: having rated the 75 easy diseased cases as fives the observer is left with 25 difficult diseased cases and 100 easy non-diseased cases. The guessing observer may randomly rate 1/5th of the remaining cases as fours. This would correctly classify 5 of the actually diseased cases (25/5) and 20 non-diseased ones (100/5). Therefore, the total number of diseased cases rated four or higher is 80 and the corresponding number of non-diseased cases is 20. The new operating point of the observer is (0.20, 0.80). One has two operating points, the original one on the y-axis at (0, 0.75) and an interior point (0.20, 0.80). Next, instead of randomly rating 1/5th of the remaining cases as fours, the observer rates 2/5th of them as fours resulting in an additional interior point (0.40, 0.85). By simply increasing the fraction of remaining cases that are randomly rated fours, the observer can move the operating point along the straight line connecting (0, 0.75) and (1, 1), as in plot D in Fig. \@ref(fig:binormal-model-degeneracy). Since it involves guessing this must represent a lower bound on performance. The lowest possible performance yields a unique ROC curve: any higher ROC curve would not be unique. 
