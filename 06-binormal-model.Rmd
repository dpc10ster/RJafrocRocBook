...
The un-normalized partial-area measure $A_c^{X}$ = `r simplePrint(A_x)`. The corresponding full AUC measure is $A_z$ = `r simplePrint(A_z)`. The normalized measure is $A_c^{XN}$ = `r simplePrint(A_xn)`. This is the classification accuracy between non-diseased and diseased cases in the truncated dataset defined by ignoring cases with z-samples smaller than $-\Phi^{-1}(c)$ = `r simplePrint(-qnorm(fpf_c))`. This measure emphasizes specificity.
