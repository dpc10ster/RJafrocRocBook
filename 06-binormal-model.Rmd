...
## Az and d' measures {#binormal-model-full-auc}
...
Shorthand: denote $A \equiv A_z$, $x \equiv \text{FPF}$ and $y \equiv \text{TPF}$ and the subscript B denotes the integral *below* the ROC (another index using the integral above the ROC will be introduced shortly). The partial area below the ROC, $A_c^{B}$, is defined as that extending from $x = 0$ to $x = c$, where $0 \le c \le 1$ (in our notation $c$ always means a cutoff applied to the x-axis of the ROC): 

\begin{equation} 
\left. 
\begin{aligned}
A_c^{B} &= \int_{x=0}^{x=c} y \, dx 
...
\begin{equation}
A_c^{B} = \int_{z_2=-\infty}^{\Phi^{-1}\left ( c \right )}   \int_{z_1=-\infty}^{\frac{a}{\sqrt{1+b^2}}} \phi\left ( z_1,z_2;\rho \right ) dz_1dz_2
...
\begin{equation} 
A_c^{A} = \int_{y=\text{TPF}(c)}^{y=1} x \, dy
(\#eq:binormal-model-partial-area-a2)
\end{equation}


Note that this is the (un-normalized) area *above* (hence the A subscript) the ROC extending from $y = \text{TPF}(c)$ to $y = 1$. The maximum value of this integral is the area of the rectangle defined by the corner points $(c,\text{TPF}(c))$ and $(1,1)$. Therefore the normalized area is defined by: 


\begin{equation}
A_c^{AN} = 1-\frac{A_c^{A}}{\left (1 - \text{TPF}(c)  \right ) \times \left (1-c  \right )}
(\#eq:binormal-model-normalized-partial-auc-upper)
\end{equation}

A *physical* meaning is as follows: 

> A binormal model based ROC curve can be defined over the truncated dataset where all z-samples **greater** than $-\Phi^{-1}(c)$ are ignored. **$A_c^{AN}$ is the classification accuracy between diseased and non-diseased cases measured over the truncated dataset.** By definition the normalized area ranges between 0 and 1.



```{r binormal-model-partial-areas, fig.cap = "Un-normalized partial AUC measures: : the blue area is $A_c^{B}$, the area below, and the green shaded area is $A_c^{A}$ the area above the ROC. Parameters are $\\mu = 1.8$, $\\sigma = 1$ and $c = 0.3$.", fig.show='hold', echo=FALSE}
print(shadedPlotsRoc(mu = 1.8, sigma = 1, fpf = 0.3))
```


Fig. \@ref(fig:binormal-model-partial-areas) shows the two un-normalized area .
