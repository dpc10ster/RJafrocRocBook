...
A partial area under the ROC can be defined as that extending from $\text{FPF} = 0$ to $\text{FPF} = c$, where $c \le 1$. This scalar measure, denoted $A_{z;c}$, obviously depends on the choice of operating point, i.e., it depends on $c$. The threshold $\zeta_1$ corresponding to $\text{FPF} = c$ is given by:
...
(As a special case, namely $c=1$, i.e., $\zeta_1=-\infty$, it yields the full area under the ROC, usually denoted $A_z$, discussed below.) 

The partial-area under the ROC curve from $\text{FPF} = 0$ to $\text{FPF} = c$ is defined by: 

\begin{equation} 
\left. 
\begin{aligned}
A_{z;c} &= \int_{\text{FPF}=0}^{\text{FPF}=c} \text{TPF} \, d\left (\text{FPF}  \right ) 
\\&=  \int_{\text{FPF}=0}^{\text{FPF}=c} \Phi\left ( a + b \Phi^{-1} \left ( \text{FPF} \right ) \right ) \, d\left (\text{FPF}  \right ) 
\end{aligned}
\right \}
(\#eq:binormal-model-partial-area-a1)
\end{equation}

See Eqn. \@ref(eq:binormal-model-roc-curve) for explanation of the second equation.

Section \@ref(binormal-model-partial-auc-derivation) derives a formula for the partial-area. The final result is:
...
On the right hand side $\phi\left ( z_1,z_2;\rho \right )$ is the standard bivariate normal distribution function with correlation coefficient $\rho$ defined by:
...
The two-dimensional integral occurring on the right hand side of Eqn. \@ref(eq:binormal-model-partial-area-final) can be evaluated numerically. 


### Full AUC {#binormal-model-full-auc}

A special case of the partial AUC formula is the full area under the ROC curve obtained by setting $\zeta_1=-\infty$ in Eqn. \@ref(eq:binormal-model-partial-area-final). This is shown next using both parameterizations of the binormal model:
...
## More on the partial area measure {#binormal-model-partial-auc-more}

The following code illustrates calculation of the partial-area measure using the function `pmvnorm` in `R` package `mvtnorm`. The following parameter values were used: $a = 2$, $b = 1$ and $\zeta_1 = 1.5$. (The parameter $b = 1$ was deliberately chosen so that we do not have to worry about improper ROC curves.)
...
The partial-area measure is $A_{z;c}$ = `r A_zc`. The corresponding full-area measure is $A_z$ = `r A_z`. $A_{z;c}$ is small because the reporting threshold $\zeta_1 = 1.5$ is high. $A_{z;c}$ should not be confused with true performance of the observer, as shown in Section \@ref(binormal-model-partial-true).


### Meaning of partial AUC {#binormal-model-meaning-partial-auc}

The partial AUC measure $A_{z;c}$ has a simple *geometric* meaning, namely as in integral of the ROC curve from $\text{FPF} = 0$ to $\text{FPF} = c$. A *physically* meaningful quantity can be derived as follows. *Consider the truncated dataset where all z-samples smaller than $\zeta_1$ are ignored*. A binormal model based ROC curve can be defined over the truncated dataset and the area under it is a normalized partial area measure, denoted $A_{z;c;norm}$. This quantity is the classification accuracy between diseased and non-diseased cases measured over the truncated dataset. 
...
In this equation $\text{TPF}_c$ is the ordinate and $\text{FPF}_c$ is the abscissa at the chosen value of $c$.

As $\zeta_1 \rightarrow -\infty$ the three measures ($A_z$, $A_{z;c}$ and $A_{z;c;norm}$), become identical.


### An alternative partial AUC measure {#binormal-model-metz-partial-auc}

Since the integral in Eqn. \@ref(eq:binormal-model-partial-area-a1) is from $\text{FPF} = 0$ to $\text{FPF} = c$ the partial AUC measure as defined there can be thought of as emphasizing the *high specificity* region of the ROC curve (since $\text{FPF} = 0$ corresponds to unit specificity). An alternative partial AUC measure has been defined [@jiang1996receiver] that emphasizes the *high sensitivity* region of the ROC as follows:

\begin{equation} 
A_{z;d} = \int_{\text{TPF}=c'}^{\text{TPF}=1} \text{FPF} \, d\left (\text{TPF}  \right )
(\#eq:binormal-model-partial-area-a2)
\end{equation}

Note that this is the (un-normalized) area *above* the ROC extending from $\text{TPF} = d$ to $\text{TPF} = 1$, where $0 \le d \le 1$, i.e., only  sensitivity values above d contribute to the integral. The maximum value of this integral is the area of the rectangle defined by the corner points $(\text{FPF}_d,d)$ and $(1,1)$. Therefore the normalized area is defined by: 


\begin{equation}
A_{z;d;u;norm} = \frac{A_{z;d;u}}{\left (1 - \text{TPF}_d  \right ) \times \left (1-\text{FPF}_d  \right )}
