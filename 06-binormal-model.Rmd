...
Since a partial area measure corresponds to classification accuracy measured over a **truncated** dataset the correspondence between $A_z$ and classification accuracy measured over the **entire** dataset is lost. The basic statistical principle of estimating over the entire population is being violated. 

The choice of the truncation cutoff is arbitrary and subject to bias on the part of the investigator, similar to the type of bias that is inherent in a single point (sensitivity-specificity) based approach to analysis: after all this was the very reason for adoption of a measure such as $A_z$ that averages over the whole curve, as argued so eloquently in [@metz1978rocmethodology]. 
