...
A partial area under the ROC can be defined as that extending from $\text{FPF} = 0$ to $\text{FPF} = c$, where $c \le 1$. This scalar measure, denoted $A_{z;c}$, depends on the choice of operating point. The threshold $\zeta_1$ corresponding to $\text{FPF} = c$ is given by:
...
(As a special case, namely $c=1$, it yields the full area under the ROC, usually denoted $A_z$, discussed below.) 

Section \@ref(binormal-model-partial-auc-derivation) derives a formula for the partial-area using the unequal-variance binormal model. The final result is:
